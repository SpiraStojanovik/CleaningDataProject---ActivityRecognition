## make sure the following data frames and are in the working folder:
...
setwd("C:/Documents and Settings/piko/Desktop/Coursera/00 R wd/data")
...
## these are the steps to extract measurements that have both mean and std value
...
## finally remove the single dots
## NOT SURE ABOUT THIS colnames(dataExtr)<-gsub(".","",make.names(colnames(dataExtr)),fixed=TRUE)
...
## replace numbers in activity with names by merging the tables (partly done in STEP 3)
##dataLab1 = merge(labels, dataExtr, by.x ="activityID", by.y="activityID", all=TRUE)
